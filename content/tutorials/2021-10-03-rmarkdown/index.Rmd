...
categories: 
- tutorials
excerpt: "Emacs for Bioinformatics: RMarkdown"
date: "2021-10-03"
updated: 2021-10-05
tags: 
- emacs
...
- R
- RMarkdown
title: "Emacs for Bioinformatics #4: RMarkdown"
...
This is part four in my series of Emacs tutorials aimed at bioinformatics
...
`polymode-next-chunk`/`polymode-previous-chunk`, bound to `M-n C-n` and `M-n C-p`

: move to the next/previous chunk. i.e., move from an RMarkdown chunk to
  the next R code chunk.

`polymode-next-chunk-same-type`/`polymode-previous-chunk-same-type`, bound to `M-n M-C-n` and `M-n M-C-p`
...
: evaluate all code chunks in the buffer

`polymode-eval-buffer-from-beg-to-point`/`polymode-eval-buffer-from-point-to-end`, bound to `M-n u` or `M-n ↑`, and `M-n d` or `M-n ↓`

: evaluate all code chunks from the beginning of the buffer to point (`u` and `↑`), or from point to the end of the buffer (`d` and `↓`)
