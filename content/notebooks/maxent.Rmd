...
@MerowEtAl_2013 provides a very thorough introduction to Maxent modeling, and especially to what the various settings mean and how to set them.

[RSpatial](https://rspatial.org/) is a (nearly complete) set of lessons covering spatial data analysis in R, and including good tutorials for Maxent and other SDM approaches.

[Best Practices in Species Distribution Modeling in R](http://www.earthskysea.org/best-practices-in-species-distribution-modeling-a-workshop-in-r/) is another set of online notes. I haven't reviewed them yet, but it looks very promising.

From the same author (Adam B. Smith), a [webinar on using herbarium data in ecology, evolution and conservation](http://www.earthskysea.org/museum-herbarium-data/), and a [database of biodiversity databases](https://docs.google.com/spreadsheets/d/1qsK1MNLsi-QjK9VptgeV2HfrB7mWUVxjZdaj_P9Qs-Y/edit#gid=975168115).
...
@BoyceEtAl_2002 proposed an index that compares the predicted and expected number of occupied sites with the suitability value of those sites:
